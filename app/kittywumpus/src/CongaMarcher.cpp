...
namespace Boidsish {
	...
	void CongaMarcher::UpdateEntity(const EntityHandler& handler, float time, float delta_time) {
		...
		// Don't attack grounded player (FPS mode)
		if (plane->IsGrounded()) {
			// Fly away and despawn
			auto current_pos = GetPosition().Toglm();
			glm::vec3 away_vel = glm::vec3(0, 50.0f, 0);
			SetVelocity(Vector3(away_vel.x, away_vel.y, away_vel.z));
			if (current_pos.y > 300.0f) {
				handler.QueueRemoveEntity(id_);
			}
			return;
		}
	...
	void CongaMarcher::OnHit(const EntityHandler& handler, float damage, const glm::vec3& hit_point) {
		if (is_dying_) return;
		health_ -= damage;
		...
		if (health_ <= 0) {
			...
			handler.EnqueueVisualizerAction([pos, &handler, this]() {
				...
				handler.vis->AddFireEffect(pos, FireEffectStyle::Cinder, glm::vec3(0, 1, 0), glm::vec3(0, 1, 0), 50, 1.0f);
			});
