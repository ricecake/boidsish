...
namespace Boidsish {
	...
	void Trail::AppendToGeometryCache(
		...
		for (size_t i = start_idx; i < curve_positions.size(); ++i) {
			...
			ring_positions.push_back(rpos);
			ring_normals.push_back(rnorm);
		}
		...
		for (size_t i = start_tri_idx; i < num_rings - 1; ++i) {
			const auto& r1p = ring_positions[i];
			const auto& r1n = ring_normals[i];
			const auto& r2p = ring_positions[i + 1];
			const auto& r2n = ring_normals[i + 1];
	...
	void Trail::UpdateMesh() {
		...
		while (num_finalized_segments < total_finalizable) {
			...
			Vector3 p0, p1, p2, p3;
			Vector3 c0, c1, c2, c3;
			p1 = Vector3(points[i].first);
			p2 = Vector3(points[i + 1].first);
			p3 = Vector3(points[i + 2].first);
			c1 = Vector3(points[i].second);
			c2 = Vector3(points[i + 1].second);
			c3 = Vector3(points[i + 2].second);
			if (i == 0) {
				p0 = p1 - (p2 - p1);
				c0 = c1;
			} else {
				p0 = Vector3(points[i - 1].first);
				c0 = Vector3(points[i - 1].second);
			}
			AppendToGeometryCache(p0, p1, p2, p3, c0, c1, c2, c3);
			permanent_tail = tail;
			num_finalized_segments++;
		}

		if (points.size() >= 2) {
			std::vector<Vector3> temp_curve;
			std::vector<Vector3> temp_colors;
			for (size_t i = num_finalized_segments; i < points.size() - 1; ++i) {
				...
				if (i == points.size() - 2) {
					temp_curve.push_back(p2);
					temp_colors.push_back(c2);
				}
			}
			...
			if (!ring_positions.empty()) {
				prev_rpos = ring_positions.back();
				prev_rnorm = ring_normals.back();
			}

			for (size_t i = 0; i < temp_curve.size(); ++i) {
				Vector3 tangent;
				if (i == 0 && last_tangent.Magnitude() < 1e-6f) {
					tangent = (temp_curve.size() > 1) ? (temp_curve[1] - temp_curve[0]).Normalized() : Vector3(0, 0, 1);
				} else if (i == 0) {
					tangent = last_tangent;
				} else {
					tangent = (temp_curve[i] - temp_curve[i - 1]).Normalized();
				}
				...
				if (!prev_rpos.empty()) {
					const auto& col = (i == 0) ? (curve_colors.empty() ? temp_colors[0] : curve_colors.back())
											   : temp_colors[i - 1];
					for (int j = 0; j <= TRAIL_SEGMENTS; ++j) {
