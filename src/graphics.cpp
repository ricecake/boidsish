...
namespace Boidsish {
	...
	std::shared_ptr<HudIcon> Visualizer::AddHudIcon(const std::string& path, HudAlignment alignment, glm::vec2 position, glm::vec2 size) {
		return impl->hud_manager->AddIcon(path, alignment, position, size);
	}

	std::shared_ptr<HudNumber> Visualizer::AddHudNumber(float value, const std::string& label, HudAlignment alignment, glm::vec2 position, int precision) {
		return impl->hud_manager->AddNumber(value, label, alignment, position, precision);
	}

	std::shared_ptr<HudGauge> Visualizer::AddHudGauge(float value, const std::string& label, HudAlignment alignment, glm::vec2 position, glm::vec2 size) {
	...
	std::shared_ptr<HudIconSet> Visualizer::AddHudIconSet(const std::vector<std::string>& paths, HudAlignment alignment, glm::vec2 position, glm::vec2 size, float spacing) {
