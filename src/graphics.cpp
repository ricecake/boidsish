...
namespace Boidsish {
	...
	struct Visualizer::VisualizerImpl {
		...
		VisualizerImpl(int w, int h, const char* title) : width(w), height(h) {
			...
			float quad_vertices[] = { -1.0f, 0.0f, -1.0f, 1.0f, 0.0f, -1.0f, 1.0f, 0.0f, 1.0f, 1.0f, 0.0f, 1.0f, -1.0f, 0.0f, 1.0f, -1.0f, 0.0f, -1.0f };
		...
		void ProcessInput(float delta_time) {
			if (auto_camera_mode) return;
			float camera_speed = 5.0f * delta_time;
			glm::vec3 front(cos(glm::radians(camera.pitch)) * sin(glm::radians(camera.yaw)), sin(glm::radians(camera.pitch)), -cos(glm::radians(camera.pitch)) * cos(glm::radians(camera.yaw)));
			glm::vec3 right = glm::normalize(glm::cross(front, glm::vec3(0.0f, 1.0f, 0.0f)));
			if (keys[GLFW_KEY_W]) { camera.x += front.x * camera_speed; camera.y += front.y * camera_speed; camera.z += front.z * camera_speed; }
			if (keys[GLFW_KEY_S]) { camera.x -= front.x * camera_speed; camera.y -= front.y * camera_speed; camera.z -= front.z * camera_speed; }
			if (keys[GLFW_KEY_A]) { camera.x -= right.x * camera_speed; camera.z -= right.z * camera_speed; }
			if (keys[GLFW_KEY_D]) { camera.x += right.x * camera_speed; camera.z += right.z * camera_speed; }
			if (keys[GLFW_KEY_SPACE]) camera.y += camera_speed;
			if (keys[GLFW_KEY_LEFT_SHIFT]) camera.y -= camera_speed;
		}
		...
		static void KeyCallback(GLFWwindow* w, int key, int sc, int action, int mods) {
			...
			if (key >= 0 && key < 1024) {
				if (action == GLFW_PRESS) impl->keys[key] = true;
				else if (action == GLFW_RELEASE) impl->keys[key] = false;
			}
			...
			if (key == GLFW_KEY_0 && action == GLFW_PRESS) {
				...
				if (!impl->auto_camera_mode) impl->first_mouse = true;
			}
			if (key == GLFW_KEY_9 && action == GLFW_PRESS) impl->single_track_mode = true;
			if (key == GLFW_KEY_8 && action == GLFW_PRESS) impl->single_track_mode = false;
		}

		static void MouseCallback(GLFWwindow* w, double xpos, double ypos) {
			auto* impl = static_cast<VisualizerImpl*>(glfwGetWindowUserPointer(w));
			if (impl->auto_camera_mode) return;
			if (impl->first_mouse) { impl->last_mouse_x = xpos; impl->last_mouse_y = ypos; impl->first_mouse = false; }
			float xoffset = xpos - impl->last_mouse_x, yoffset = impl->last_mouse_y - ypos;
			impl->last_mouse_x = xpos; impl->last_mouse_y = ypos;
			xoffset *= 0.1f; yoffset *= 0.1f;
			impl->camera.yaw += xoffset; impl->camera.pitch += yoffset;
			if (impl->camera.pitch > 89.0f) impl->camera.pitch = 89.0f;
			if (impl->camera.pitch < -89.0f) impl->camera.pitch = -89.0f;
		}
	...
	void Visualizer::SetShapeHandler(ShapeFunction func) { impl->shape_function = func; }
	bool Visualizer::ShouldClose() const { return glfwWindowShouldClose(impl->window); }
	...
	void Visualizer::Render() {
		...
		if (impl->shape_function) shapes = impl->shape_function(time);
	...
	void Visualizer::Run() { while (!ShouldClose()) { Update(); Render(); } }
	Camera& Visualizer::GetCamera() { return impl->camera; }
	void Visualizer::SetCamera(const Camera& camera) { impl->camera = camera; }
}
