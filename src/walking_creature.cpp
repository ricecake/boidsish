...
namespace Boidsish {
	...
	WalkingCreature::WalkingCreature(int id, float x, float y, float z, float length):
		...
		body_node_idx_ = AddVertex(Vector3(0, height_, 0), length * 5.0f, 0.8f, 0.4f, 0.2f, 1.0f).GetId();

		neck_node_idx_ = AddVertex(Vector3(0, height_ + length * 2.0f, 0), length * 3.0f, 0.8f, 0.4f, 0.2f, 1.0f)
							 .GetId();
		AddEdge(V(body_node_idx_), V(neck_node_idx_));

		// Add Head Node
		head_node_idx_ =
			AddVertex(Vector3(0, height_ + length * 3.0f, length * 0.2f), length * 2.0f, 0.9f, 0.5f, 0.3f, 1.0f)
				.GetId();
		AddEdge(V(neck_node_idx_), V(head_node_idx_));
		...
		for (int i = 0; i < 4; ++i) {
			...
			// Add Knee Node
			legs_[i].knee_node_idx = AddVertex(
										 Vector3(legs_[i].rest_offset * 3.0f + glm::vec3(0, height_ * 0.5f, 0)),
										 length * 0.25f,
										 0.7f,
										 0.35f,
										 0.15f,
			...
			// Add Foot Node
			legs_[i].node_idx = AddVertex(Vector3(legs_[i].rest_offset), length * 2.35f, 0.6f, 0.3f, 0.1f, 1.0f)
									.GetId();
	...
	void WalkingCreature::UpdateMovement(float delta_time) {
		...
		glm::vec3 current_forward = glm::vec3(
			std::sin(glm::radians(current_yaw_)),
			0,
			std::cos(glm::radians(current_yaw_))
		);
		...
		for (int j = 0; j < 4; ++j) {
			...
			if (is_walking_ && leg_phi < 1.0f) { // Leg is moving
				if (!legs_[leg_idx].is_moving) {
					...
					legs_[leg_idx].step_target_pos = current_pos_ + rotated_offset +
						current_forward * (step_length * 0.75f);
				}
				...
				legs_[leg_idx].world_foot_pos = glm::mix(
					legs_[leg_idx].step_start_pos,
					legs_[leg_idx].step_target_pos,
					horizontal_p
				);
				...
				glm::mat4 rotation = glm::rotate(glm::mat4(1.0f), glm::radians(current_yaw_), glm::vec3(0, 1, 0));
				glm::vec3 body_joint_world = current_pos_ +
					glm::vec3(rotation *
				              glm::vec4(legs_[leg_idx].rest_offset.x, 0.0f, legs_[leg_idx].rest_offset.z, 0.0f));
				...
				legs_[leg_idx].world_knee_pos = mid_pos +
					glm::vec3(0, height_ * 0.3f + knee_vertical_p * knee_step_height, 0);

				// Push knee outwards slightly for the arch
				glm::vec3 out_dir = glm::normalize(
					legs_[leg_idx].world_knee_pos - (current_pos_ + glm::vec3(0, height_, 0))
				);
				...
				glm::vec3 body_joint_world = current_pos_ +
					glm::vec3(rotation *
				              glm::vec4(legs_[leg_idx].rest_offset.x, 0.0f, legs_[leg_idx].rest_offset.z, 0.0f));
				body_joint_world.y += height_;
				legs_[leg_idx].world_knee_pos = glm::mix(body_joint_world, legs_[leg_idx].world_foot_pos, 0.5f) +
					glm::vec3(0, height_ * 0.3f, 0);
			}
		}
		...
		if (light_change_timer_ <= 0.0f) {
			...
			spotlight_.color = glm::vec3(
				(std::rand() % 100) / 100.0f,
				(std::rand() % 100) / 100.0f,
				(std::rand() % 100) / 100.0f
			);
			if (glm::length(spotlight_.color) < 0.2f)
				spotlight_.color = glm::vec3(1.0f); // Ensure it's not too dark
	...
	void WalkingCreature::UpdateNodes(float delta_time) {
		...
		current_head_dir_ = glm::normalize(
			glm::mix(current_head_dir_, target_look_dir, std::min(1.0f, delta_time * lerp_speed))
		);
		...
		V(neck_node_idx_).position = (V(head_node_idx_).position - V(body_node_idx_).position) / 2.0f;
		V(neck_node_idx_).position.y = V(head_node_idx_).position.y * 1.10f;
		// Update Spotlight
