...
namespace Boidsish {
	...
	namespace {
		...
		void AddPuffball(std::vector<Vertex>& vertices, std::vector<unsigned int>& indices,
						 glm::vec3 center, float radius, glm::vec3 color, std::mt19937& gen) {
		...
		void AddLeaf(std::vector<Vertex>& vertices, std::vector<unsigned int>& indices,
					 glm::vec3 pos, glm::quat ori, float size, glm::vec3 color) {
			unsigned int base = vertices.size();

			// More rounded leaf shape (diamond with midpoints)
			std::vector<glm::vec3> pts = {
				{0, 0, 0}, {0.3f, 0.5f, 0.1f}, {0, 1.0f, 0}, {-0.3f, 0.5f, -0.1f}
			};
		...
		void AddSplineTube(std::vector<Vertex>& vertices, std::vector<unsigned int>& indices,
						   const std::vector<glm::vec3>& pts, const std::vector<float>& radii,
