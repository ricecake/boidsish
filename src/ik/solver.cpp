    ...
    for (int iter = 0; iter < maxIterations; ++iter) {
        ...
        for (auto& chain : body.tree.chains) {
            ...
            for (size_t i = 0; i < chain.bones.size(); ++i) {
                ...
                if (!bone.joints.empty()) {
                    ...
                    for (const auto& constraint : joint.constraints) {
                        if (constraint.type == ConstraintType::Hinge) {
                            glm::vec3 referenceAxis = glm::vec3(0, 1, 0);
                            if (i > 0) {
                                referenceAxis = glm::normalize(chain.bones[i-1].position - (i > 1 ? chain.bones[i-2].position : body.position + chain.base));
                            }
                            bone.position = ApplyHingeConstraint(bone.position, prevPos, constraint.axis, referenceAxis, constraint.minAngle, constraint.maxAngle, length);
                        } else if (constraint.type == ConstraintType::Cone) {
                             glm::vec3 coneAxis = glm::vec3(0, 1, 0);
                             if (i > 0) {
                                 coneAxis = glm::normalize(chain.bones[i-1].position - (i > 1 ? chain.bones[i-2].position : body.position + chain.base));
                             }
                             bone.position = ApplyConeConstraint(bone.position, prevPos, coneAxis, constraint.angle, length);
                        }
                    }
                }
                ...
                bone.orientation = glm::rotation(glm::vec3(0, 1, 0), newDir);
                ...
                if (!bone.joints.empty()) {
                    ...
                    for (const auto& constraint : joint.constraints) {
                        if (constraint.type == ConstraintType::Twist) {
                            glm::vec3 parentUp = glm::vec3(0, 0, 1);
                            if (i > 0) {
                                parentUp = chain.bones[i-1].orientation * glm::vec3(0, 1, 0);
                            }
                            ApplyTwistConstraint(bone.orientation, newDir, parentUp, constraint.angle);
                        }
                    }
                }
