...
DeformationQueryResult TerrainDeformationManager::QueryDeformations(
	float x, float z, float current_height, const glm::vec3& current_normal
) const {
	...
	for (uint32_t id : affecting_ids) {
		...
		if (it == deformations_.end()) continue;

		const auto& deformation = it->second;
		if (!deformation->ContainsPointXZ(x, z)) continue;
...
std::vector<std::shared_ptr<TerrainDeformation>> TerrainDeformationManager::GetDeformationsAt(float x, float z) const {
	std::shared_lock lock(mutex_);
...
std::vector<std::shared_ptr<TerrainDeformation>> TerrainDeformationManager::FindDeformationsInAABB(
	const glm::vec3& min_corner, const glm::vec3& max_corner
) const {
	std::shared_lock lock(mutex_);
...
bool TerrainDeformationManager::ChunkHasDeformations(
	float chunk_min_x, float chunk_min_z, float chunk_max_x, float chunk_max_z
) const {
	std::shared_lock lock(mutex_);
...
void TerrainDeformationManager::PopulateVoxelsForDeformation(
	const std::shared_ptr<TerrainDeformation>& deformation
) {
