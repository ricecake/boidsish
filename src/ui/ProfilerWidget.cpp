    ...
    namespace UI {
        ...
        void ProfilerWidget::Draw() {
            ...
            if (ImGui::Begin("Profiler", &m_show)) {
                ...
                if (ImGui::BeginTable("ProfilerStats", 5, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_Resizable | ImGuiTableFlags_Sortable)) {
                            ...
                            std::sort(sortedStats.begin(), sortedStats.end(), [&](const auto& a, const auto& b) {
                                for (int n = 0; n < sort_specs->SpecsCount; n++) {
                                    ...
                                    switch (spec->ColumnIndex) {
                                        case 0: delta = a.first.compare(b.first); break;
                                        case 1: delta = (a.second.count < b.second.count) ? -1 : (a.second.count > b.second.count) ? 1 : 0; break;
                                        case 2: delta = (a.second.GetAverageUs() < b.second.GetAverageUs()) ? -1 : (a.second.GetAverageUs() > b.second.GetAverageUs()) ? 1 : 0; break;
                                        case 3: delta = (a.second.minTimeUs < b.second.minTimeUs) ? -1 : (a.second.minTimeUs > b.second.minTimeUs) ? 1 : 0; break;
                                        case 4: delta = (a.second.maxTimeUs < b.second.maxTimeUs) ? -1 : (a.second.maxTimeUs > b.second.maxTimeUs) ? 1 : 0; break;
                                    }
                                    if (delta != 0) {
