...
namespace Boidsish {
	namespace UI {
		LightingWidget::LightingWidget(LightManager& lightManager): _lightManager(lightManager) {
			m_show = true;
		}
		void LightingWidget::Draw() {
			...
			// 1. Dropdowns (Dynamic Lights)
			for (int i = 0; i < (int)lights.size(); ++i) {
				ImGui::PushID(i);
				if (ImGui::TreeNode("Light", "Light %d", i)) {
					// 3. Sliders (Position, Direction, Color, Intensity)
					...
					if (lights[i].type == SPOT_LIGHT) {
						float inner = glm::degrees(glm::acos(lights[i].inner_cutoff));
						float outer = glm::degrees(glm::acos(lights[i].outer_cutoff));
						if (ImGui::DragFloat("Inner Angle", &inner, 0.1f, 0.0f, 90.0f)) {
							if (inner > outer) inner = outer;
							lights[i].inner_cutoff = glm::cos(glm::radians(inner));
						}
						if (ImGui::DragFloat("Outer Angle", &outer, 0.1f, 0.0f, 90.0f)) {
							if (inner > outer) outer = inner;
							lights[i].outer_cutoff = glm::cos(glm::radians(outer));
						}
					}

					// 4. Combos (Type, Behavior) - LAST in dropdown
					const char* types[] = {"Point", "Directional", "Spot"};
					int current_type = (int)lights[i].type;
					if (ImGui::Combo("Type", &current_type, types, IM_ARRAYSIZE(types))) lights[i].type = (LightType)current_type;
					...
					if (ImGui::Combo("Behavior", &current_behavior, behaviors, IM_ARRAYSIZE(behaviors))) {
						...
						if (lights[i].behavior.type == LightBehaviorType::MORSE) lights[i].behavior.morse_index = -1;
					}

					if (lights.size() > 1 && ImGui::Button("Remove")) light_to_remove = i;
					ImGui::TreePop();
				}
				ImGui::PopID();
			}
			if (light_to_remove != -1) lights.erase(lights.begin() + light_to_remove);
			ImGui::Separator();

			// 3. Sliders (Ambient Light)
			glm::vec3 ambient = _lightManager.GetAmbientLight();
			if (ImGui::ColorEdit3("Ambient Light", &ambient[0])) _lightManager.SetAmbientLight(ambient);

			if (ImGui::Button("Add Light")) _lightManager.AddLight(Light::Create({0, 10, 0}, 5.0f, {1, 1, 1}, false));
			ImGui::End();
		}
	}
}
