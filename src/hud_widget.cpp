...
HudWidget::HudWidget(HudManager& hudManager)
...
void HudWidget::Draw() {
    ...
    for (const auto& icon : icons) {
        ...
        if (textureId != 0) {
            ImVec4 tint_col = icon.highlighted ? ImVec4(1.0f, 1.0f, 0.0f, 1.0f) : ImVec4(1.0f, 1.0f, 1.0f, 1.0f);
            logger::LOG("Drawing icon " + std::to_string(icon.id));
            ImGui::Image((void*)(intptr_t)textureId, size, ImVec2(0, 0), ImVec2(1, 1), tint_col, ImVec4(0,0,0,0));
    ...
    for (const auto& number : numbers) {
        char buffer[64];
        snprintf(buffer, sizeof(buffer), ("%s: %." + std::to_string(number.precision) + "f").c_str(), number.label.c_str(), number.value);
