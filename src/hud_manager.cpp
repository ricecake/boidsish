...
namespace Boidsish {
	...
	// Modern API
	std::shared_ptr<HudIcon> HudManager::AddIcon(const std::string& path, HudAlignment alignment, glm::vec2 position, glm::vec2 size) {
		auto icon = std::make_shared<HudIcon>(path, alignment, position, size);
		m_elements.push_back(icon);
		return icon;
	}

	std::shared_ptr<HudNumber> HudManager::AddNumber(float value, const std::string& label, HudAlignment alignment, glm::vec2 position, int precision) {
		auto number = std::make_shared<HudNumber>(value, label, alignment, position, precision);
		m_elements.push_back(number);
		return number;
	}

	std::shared_ptr<HudGauge> HudManager::AddGauge(float value, const std::string& label, HudAlignment alignment, glm::vec2 position, glm::vec2 size) {
	...
	void HudManager::RemoveIcon(int id) {
		m_elements.erase(
			std::remove_if(m_elements.begin(), m_elements.end(), [id](const std::shared_ptr<HudElement>& e) {
				return e->GetId() == id;
			}),
			m_elements.end()
		);
	...
	void HudManager::AddNumber(const HudNumber& number) {
		auto newNumber = AddNumber(number.GetValue(), number.GetLabel(), number.GetAlignment(), number.GetPosition(), number.GetPrecision());
	...
	void HudManager::AddGauge(const HudGauge& gauge) {
		auto newGauge = AddGauge(gauge.GetValue(), gauge.GetLabel(), gauge.GetAlignment(), gauge.GetPosition(), gauge.GetSize());
	...
	void HudIcon::Draw(HudManager& manager) {
		...
		if (textureId != 0) {
			...
			ImGui::Image((void*)(intptr_t)textureId, {size.x, size.y}, ImVec2(0, 0), ImVec2(1, 1), tint_col, ImVec4(0, 0, 0, 0));
		}
	}
	...
	void HudScore::Draw(HudManager& /*manager*/) {
		...
		for (const auto& change : m_changes) {
			...
			glm::vec2 cPos = HudManager::GetAlignmentPosition(m_alignment, {cTextSize.x, cTextSize.y}, {m_position.x, m_position.y + yOffset});
