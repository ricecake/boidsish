...
namespace Boidsish {
	...
	void HudManager::RemoveIcon(int id) {
		m_icons.erase(
			std::remove_if(m_icons.begin(), m_icons.end(), [id](const HudIcon& i) { return i.id == id; }),
			m_icons.end()
		);
	}
	...
	void HudManager::RemoveNumber(int id) {
		m_numbers.erase(
			std::remove_if(m_numbers.begin(), m_numbers.end(), [id](const HudNumber& n) { return n.id == id; }),
			m_numbers.end()
		);
	}
	...
	void HudManager::RemoveGauge(int id) {
		m_gauges.erase(
			std::remove_if(m_gauges.begin(), m_gauges.end(), [id](const HudGauge& g) { return g.id == id; }),
			m_gauges.end()
		);
	}
