...
namespace Boidsish {
	...
	void RustleAudioEffect::OnUpdate(float deltaTime, const AudioState& state) {
		...
		float strength = std::clamp(state.wind_strength, 0.0f, 1.0f);
		float targetGain = strength * state.grass_density * 0.25f;
		timeStep += deltaTime;

		// Interpolate gain
		float currentGain = m_gain.load(std::memory_order_relaxed);
		float nextGain = currentGain + (targetGain - currentGain) * deltaTime * 2.0f * (Simplex::noise(timeStep) * 0.5 + 0.5);
