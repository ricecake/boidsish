...
namespace Boidsish {
	...
	SteeringProbe::SteeringProbe(std::shared_ptr<ITerrainGenerator> terrain):
		terrain_(terrain) {}

	void SteeringProbe::Update(
		float            dt,
		...
		if (heightAbove < 0.0f) liftForce += 100.0f; // Stronger push if below ground
	...
	void SteeringProbe::HandleCheckpoints(float dt, EntityHandler& handler, std::shared_ptr<EntityBase> player) {
		...
		if (glm::length(velocity_) < 0.1f) return;
		...
		if (timeTrigger || distTrigger || turnTrigger) {
			...
			if (std::abs(glm::dot(currentDir, up)) > 0.99f) up = glm::vec3(1, 0, 0);

			// Ring faces BACKWARDS so player coming from behind goes through it
			glm::quat rotation = glm::quatLookAt(-currentDir, up);

			auto id = handler.AddEntity<CheckpointRing>(radius, style, [](float d, std::shared_ptr<EntityBase> e) {
			...
			if (ring) {
				ring->SetPosition(position_.x, position_.y, position_.z);
