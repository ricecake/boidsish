...
namespace Boidsish {
	...
	bool TrailRenderManager::RegisterTrail(int trail_id, size_t max_points) {
		...
		if (trail_allocations_.count(trail_id)) return false;
	...
	void TrailRenderManager::UpdateTrailData(
		...
		std::lock_guard<std::mutex> lock(mutex_);
		auto it = trail_allocations_.find(trail_id);
		if (it == trail_allocations_.end()) return;
	...
	void TrailRenderManager::EnsureBufferCapacity(size_t required_points) {
		if (required_points <= points_capacity_) return;
		size_t new_capacity = static_cast<size_t>(points_capacity_ * GROWTH_FACTOR);
		while (new_capacity < required_points) new_capacity = static_cast<size_t>(new_capacity * GROWTH_FACTOR);
	...
	void TrailRenderManager::CommitUpdates() {
		std::lock_guard<std::mutex> lock(mutex_);
		if (trail_allocations_.empty()) return;
		...
		for (auto& [id, data] : pending_point_data_) {
			...
			if (it != trail_allocations_.end()) {
				glBufferSubData(GL_SHADER_STORAGE_BUFFER, it->second.points_offset * 8 * sizeof(float), data.size() * sizeof(float), data.data());
			}
		}
		...
		for (const auto& [id, alloc] : trail_allocations_) {
			...
			uint32_t flags = 0;
			if (alloc.iridescent) flags |= 1;
			if (alloc.rocket_trail) flags |= 2;
			if (alloc.use_pbr) flags |= 4;
