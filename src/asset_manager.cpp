...
namespace Boidsish {
	...
	namespace {
		...
		std::vector<Texture> LoadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName, ModelData& data, const std::string& directory);
		...
		Mesh ProcessMesh(aiMesh* mesh, const aiScene* scene, ModelData& data, const std::string& directory) {
			...
			std::vector<Texture> diffuseMaps = LoadMaterialTextures(material, aiTextureType_DIFFUSE, "texture_diffuse", data, directory);
			textures.insert(textures.end(), diffuseMaps.begin(), diffuseMaps.end());
			std::vector<Texture> specularMaps = LoadMaterialTextures(material, aiTextureType_SPECULAR, "texture_specular", data, directory);
			textures.insert(textures.end(), specularMaps.begin(), specularMaps.end());
			std::vector<Texture> normalMaps = LoadMaterialTextures(material, aiTextureType_HEIGHT, "texture_normal", data, directory);
			textures.insert(textures.end(), normalMaps.begin(), normalMaps.end());
			std::vector<Texture> heightMaps = LoadMaterialTextures(material, aiTextureType_AMBIENT, "texture_height", data, directory);
	...
	std::shared_ptr<ma_resource_manager_data_source> AssetManager::GetAudioDataSource(const std::string& path, ma_engine* engine) {
		...
		ma_result result = ma_resource_manager_data_source_init(pResourceManager, path.c_str(), 0, NULL, dataSource.get());
