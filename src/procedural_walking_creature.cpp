...
namespace Boidsish {
	...
	void ProceduralWalkingCreature::Update(float delta_time) {
		...
		// Adjust body height based on terrain and gait oscillation
		float oscillation = 0;
		if(is_walking_) oscillation = std::sin(walk_phi_ * std::numbers::pi_v<float> * 2.0f) * length_ * 0.05f;
		current_pos_.y = terrain_h + oscillation + height_ * 0.4f;
		...
		for (auto& leg : legs_) {
			...
			// Clamp foot to terrain
			glm::vec3 target = leg.world_foot_pos;
			if (terrain_gen) {
				auto [fh, fnorm] = terrain_gen->GetTerrainPropertiesAtPoint(target.x, target.z);
				target.y = fh;
			}
			// Add gait lift if moving
			if(leg.is_moving) {
				float p = (walk_phi_ - (float)std::floor(walk_phi_));
				// Since we move one leg at a time, its phi is [0, 1] during movement
				target.y += std::sin(p * std::numbers::pi_v<float>) * length_ * 0.25f;
			}
	...
	void ProceduralWalkingCreature::UpdateMovement(float delta_time) {
		...
		if (dist > 0.1f) {
			...
			while (yaw_diff < -180.0f) yaw_diff += 360.0f;
			current_yaw_ += yaw_diff * std::min(1.0f, delta_time * 4.0f);

			float speed = length_ * 1.5f;
		...
		for (int j = 0; j < 4; ++j) {
			...
			if (is_walking_ && leg_phi < 1.0f) {
				if (!legs_[leg_idx].is_moving) {
					...
					legs_[leg_idx].step_target_pos = current_pos_ + rotated_offset + forward * length_ * 0.8f;
				}
				legs_[leg_idx].world_foot_pos = glm::mix(legs_[leg_idx].step_start_pos, legs_[leg_idx].step_target_pos, leg_phi);
			} else {
				if (legs_[leg_idx].is_moving) {
