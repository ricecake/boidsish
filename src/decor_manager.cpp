...
namespace Boidsish {
	...
	void DecorManager::_UpdateAllocation(
		...
		if (!chunks_to_generate.empty()) {
			...
			for (size_t i = 0; i < decor_types_.size(); ++i) {
				...
				glBindBufferBase(GL_SHADER_STORAGE_BUFFER, Constants::SsboBinding::DecorInstances(), decor_types_[i].ssbo);
	...
	void DecorManager::Cull(
		...
		for (auto& type : decor_types_) {
			...
			glBindBufferBase(GL_SHADER_STORAGE_BUFFER, Constants::SsboBinding::DecorInstances(), type.ssbo);
			glBindBufferBase(GL_SHADER_STORAGE_BUFFER, Constants::SsboBinding::DecorVisibleInstances(), type.visible_ssbo);
	...
	std::vector<DecorTypeResults> DecorManager::GetDecorInChunks(
		...
		glBindBufferBase(GL_SHADER_STORAGE_BUFFER, Constants::SsboBinding::DecorInstances(), temp_instance_ssbo);
