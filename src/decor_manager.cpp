...
namespace Boidsish {
	...
	void DecorManager::UpdateCPU(
		...
		glm::vec2 rotation_dir = (rotation_magnitude > 0.001f) ? camera_rotation_delta_ / rotation_magnitude : glm::vec2(0.0f);
		...
		// 2. Mark chunks as seen; evict stale
		...
		prepared_placement_.blocks_to_validate.clear();
		for (auto it = active_chunks_.begin(); it != active_chunks_.end();) {
			...
			if (is_present) {
				...
				prepared_placement_.blocks_to_validate.push_back(-block - 1); // Negative indicates invalidate
		...
		// 4. Allocate and collect
		std::vector<int> chunks_to_generate_src_indices;
		int              chunks_new = 0;
		for (const auto& cand : candidates) {
			...
			if (it == active_chunks_.end()) {
				if (chunks_new >= kMaxNewChunksPerFrame || free_blocks_.empty())
					continue;

				int block = free_blocks_.back();
				free_blocks_.pop_back();
				active_chunks_[cd.key] = {block, cd.update_count, true, frame_counter_};
				chunks_to_generate_src_indices.push_back(cand.src_index);
				prepared_placement_.blocks_to_validate.push_back(block);
				chunks_new++;
			} else if (it->second.update_count != cd.update_count) {
				it->second.update_count = cd.update_count;
				it->second.is_dirty = true;
				chunks_to_generate_src_indices.push_back(cand.src_index);
				prepared_placement_.blocks_to_validate.push_back(it->second.block_index);
			}
		}

		// 5. Prepare GPU data
		if (chunks_to_generate_src_indices.empty()) {
			prepared_placement_.has_work = !prepared_placement_.blocks_to_validate.empty();
			prepared_placement_.chunk_gpu.clear();
			return;
		}

		int num_chunks = (int)chunks_to_generate_src_indices.size();
		prepared_placement_.globals.camera_and_scale = glm::vec4(cam_xz, world_scale, terrain_gen.GetMaxHeight());
		prepared_placement_.globals.distance_params = glm::vec4(
		...
		prepared_placement_.chunk_gpu.resize(num_chunks);
		for (int j = 0; j < num_chunks; ++j) {
			const auto& cd = all_chunks[chunks_to_generate_src_indices[j]];
			auto        it = active_chunks_.find(cd.key);
			prepared_placement_.chunk_gpu[j].offset_slice_size = glm::vec4(cd.world_offset, cd.slice, cd.chunk_size);
			prepared_placement_.chunk_gpu[j].indices = glm::ivec4(it->second.block_index * kInstancesPerChunk, 0, 0, 0);
		}
		prepared_placement_.has_work = true;
	}

	void DecorManager::UpdateGPU(
		const ITerrainGenerator&              /*terrain_gen*/,
		std::shared_ptr<TerrainRenderManager> render_manager
	) {
		PROJECT_PROFILE_SCOPE("DecorManager::UpdateGPU");
		if (!prepared_placement_.has_work)
			return;
		...
		if (!prepared_placement_.blocks_to_validate.empty()) {
			glBindBuffer(GL_SHADER_STORAGE_BUFFER, block_validity_ssbo_);
			for (int block_ref : prepared_placement_.blocks_to_validate) {
				uint32_t val = (block_ref >= 0) ? 1 : 0;
				int      idx = (block_ref >= 0) ? block_ref : (-block_ref - 1);
				glBufferSubData(GL_SHADER_STORAGE_BUFFER, idx * sizeof(uint32_t), sizeof(uint32_t), &val);
			}
		}

		if (prepared_placement_.chunk_gpu.empty()) {
			prepared_placement_.has_work = false;
			return;
		}
		...
		std::memcpy(chunk_params_pb_->GetFrameDataPtr(), prepared_placement_.chunk_gpu.data(),
			prepared_placement_.chunk_gpu.size() * sizeof(ChunkParamsGPU));
		...
		int num_chunks = (int)prepared_placement_.chunk_gpu.size();
		for (size_t i = 0; i < decor_types_.size(); ++i) {
