        ...
        if (num_vertices > 0) {
            ...
            for (const auto& contour : polygon) {
                for (size_t i = 0; i < contour.size(); ++i) {
                    ...
                    vertices.insert(vertices.end(), { p1_front.x, p1_front.y, p1_front.z, normal.x, normal.y, normal.z, 0.0f, 0.0f });
                    vertices.insert(vertices.end(), { p2_front.x, p2_front.y, p2_front.z, normal.x, normal.y, normal.z, 0.0f, 0.0f });
                    vertices.insert(vertices.end(), { p1_back.x, p1_back.y, p1_back.z, normal.x, normal.y, normal.z, 0.0f, 0.0f });

                    vertices.insert(vertices.end(), { p1_back.x, p1_back.y, p1_back.z, normal.x, normal.y, normal.z, 0.0f, 0.0f });
                    vertices.insert(vertices.end(), { p2_front.x, p2_front.y, p2_front.z, normal.x, normal.y, normal.z, 0.0f, 0.0f });
                    vertices.insert(vertices.end(), { p2_back.x, p2_back.y, p2_back.z, normal.x, normal.y, normal.z, 0.0f, 0.0f });
                }
            }
