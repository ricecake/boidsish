...
namespace Boidsish {
	...
	void ConfigManager::Initialize(const std::string& app_name) {
		...
		GetAppSettingBool("enable_shadows", true);
		GetAppSettingFloat("render_scale", 1.0f);
		...
		GetAppSettingInt("god_rays_samples", 64);
		GetAppSettingFloat("god_rays_density", 0.99f);
		GetAppSettingFloat("god_rays_weight", 0.15f);
		GetAppSettingFloat("god_rays_decay", 0.95f);
		GetAppSettingFloat("god_rays_exposure", 0.5f);
