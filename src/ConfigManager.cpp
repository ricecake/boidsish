...
namespace Boidsish {
    ...
    void ConfigManager::Initialize(const std::string& app_name) {
        ...
        std::transform(m_appSection.begin(), m_appSection.end(), m_appSection.begin(),
            [](unsigned char c){ return std::tolower(c); });
        std::replace_if(m_appSection.begin(), m_appSection.end(),
            [](unsigned char c){ return !std::isalnum(c); }, '_');

        // Remove consecutive underscores
        m_appSection.erase(std::unique(m_appSection.begin(), m_appSection.end(),
            [](char a, char b){ return a == '_' && b == '_'; }), m_appSection.end());
    }
    ...
    void ConfigManager::RegisterKey(const std::string& section, const std::string& key, ConfigValue::Type type, const std::string& default_value) {
        if (m_appSection.empty()) return;
    ...
    void ConfigManager::RegisterKey(const std::string& section, const std::string& key, ConfigValue::Type type, int default_value) {
        if (m_appSection.empty()) return;
    ...
    void ConfigManager::RegisterKey(const std::string& section, const std::string& key, ConfigValue::Type type, float default_value) {
        if (m_appSection.empty()) return;
    ...
    void ConfigManager::RegisterKey(const std::string& section, const std::string& key, ConfigValue::Type type, bool default_value) {
        if (m_appSection.empty()) return;
