...
namespace Boidsish {
    ...
    std::optional<T> InterpOpt(float t, const std::optional<T>& o0, const std::optional<T>& o1, const std::optional<T>& o2, const std::optional<T>& o3, InterpType type) {
        ...
        // If one of the endpoints is missing, we can't interpolate properly through the spline
        // For simplicity, if the main points are present, we use them, falling back to neighbors for spline guidance
    ...
    MoodSettings MoodManager::Interpolate(const MoodLayer& layer, float paramValue) {
        ...
        int i = 0;
        while (i < (int)pts.size() - 1 && pts[i+1].parameterValue < paramValue) i++;

        int i0 = std::max(0, i - 1), i1 = i, i2 = std::min((int)pts.size() - 1, i + 1), i3 = std::min((int)pts.size() - 1, i + 2);
        float t = glm::clamp((paramValue - pts[i1].parameterValue) / (pts[i2].parameterValue - pts[i1].parameterValue), 0.0f, 1.0f);

        const MoodSettings &s0 = pts[i0].settings, &s1 = pts[i1].settings, &s2 = pts[i2].settings, &s3 = pts[i3].settings;
        MoodSettings res;
