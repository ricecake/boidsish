...
namespace Boidsish {
	...
	TerrainGenerator::TerrainGenerator(int seed) : thread_pool_(), seed_(seed) {
        auto fnPerlin = FastNoise::New<FastNoise::Perlin>();
        auto fnFractal = FastNoise::New<FastNoise::FractalFBm>();
        fnFractal->SetSource(fnPerlin);
        fnFractal->SetOctaveCount(2);
        control_noise_generator_ = fnFractal;
    }
	...
	glm::vec3 TerrainGenerator::pointGenerate(float x, float z) const {
		...
		float control_value = control_noise_generator_->GenSingle2D(x * control_noise_scale_, z * control_noise_scale_, seed_);
        control_value = (control_value + 1.0f) / 2.0f;
