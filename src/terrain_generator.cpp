...
namespace Boidsish {
	...
	TerrainGenerator::TerrainGenerator(int seed): seed_(seed), thread_pool_() {
		...
		// Eg@APBBCQcQ@Dw=
		// Eg@APBBCQc@BgQR@DDA==
		// LQIVCQ8AB@CgAAIA/CQc@ABwQQQDAACAPgsAAIA/BAIJDAsAAIA/BA==  --- best so far, needs a min-smooth
	...
	glm::vec3 TerrainGenerator::pointGenerate(float x, float z) const {
		...
		auto            low_threshold = (floor(raw_biome_index) / biomes.size());
		auto            high_threshold = (ceil(raw_biome_index) / biomes.size());
