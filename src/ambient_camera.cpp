...
namespace Boidsish {
	...
	void AmbientCameraSystem::Update(
		...
		auto criticallyDampedSpring = [](glm::vec3& current, glm::vec3& velocity, glm::vec3 target, float omega,
										 float dt) {
	...
	void AmbientCameraSystem::GenerateFocusPath(ITerrainGenerator* terrain, DecorManager* decor) {
		...
		for (size_t i = 1; i < probeSpline.waypoints.size(); ++i) {
			...
			glm::vec3 toFocus = glm::normalize(glm::vec3(targetFocus.x - cPos.x, targetFocus.y - cPos.y, targetFocus.z - cPos.z));
