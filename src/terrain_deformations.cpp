...
DeformationResult
CraterDeformation::ComputeDeformation(float x, float z, float current_height, const glm::vec3& current_normal) const {
...
glm::vec3
FlattenSquareDeformation::TransformNormal(float x, float z, const glm::vec3& original_normal) const {
	glm::vec2 local = WorldToLocal(x, z);
	float     blend = ComputeBlendWeight(local.x, local.y);
