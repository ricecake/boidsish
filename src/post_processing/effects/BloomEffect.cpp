...
namespace Boidsish {
	...
	void BloomEffect::SyncState() {
		state::SyncBloomState(this);
	}

	namespace state {
		void SyncBloomState(void* effect_ptr) {
			auto effect = static_cast<PostProcessing::BloomEffect*>(effect_ptr);
			if (!effect) return;
			auto store = ServiceLocator::Instance().Get<state::Store>();
			state::BloomSettings actual;
			actual.enabled = effect->IsEnabled();
			actual.intensity = effect->GetIntensity();
			actual.threshold = effect->GetThreshold();

			auto fillLayer = [](state::BloomLayerSettings& dest, const PostProcessing::BloomEffect::LayerSettings& src) {
				dest.toneMappingEnabled = src.toneMappingEnabled;
				dest.toneMappingMode = src.toneMappingMode;
				dest.autoExposureEnabled = src.autoExposureEnabled;
				dest.targetLuminance = src.targetLuminance;
			...
			fillLayer(actual.scene, effect->GetSceneSettings());
			fillLayer(actual.sky, effect->GetSkySettings());

			store->Dispatch(state::actions::SyncBloomActual{actual});
		}
	}
} // namespace Boidsish
