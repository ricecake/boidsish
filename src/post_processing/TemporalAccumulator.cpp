...
namespace Boidsish {
	namespace PostProcessing {
		...
		void TemporalAccumulator::CreateTextures() {
			for (int i = 0; i < 2; i++) {
				_historyTextures[i] = std::make_unique<PersistentTexture>(GL_TEXTURE_2D, _internalFormat, _width, _height, 1, 1);

				GLuint id = _historyTextures[i]->GetId();
		...
		GLuint TemporalAccumulator::Accumulate(GLuint currentFrame, GLuint velocityTexture, GLuint depthTexture) {
			...
			glBindTexture(GL_TEXTURE_2D, _historyTextures[_currentIndex]->GetId());
			...
			glBindImageTexture(0, _historyTextures[nextIndex]->GetId(), 0, GL_FALSE, 0, GL_WRITE_ONLY, _internalFormat);

			glDispatchCompute((_width + 7) / 8, (_height + 7) / 8, 1);
			glMemoryBarrier(GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);

			_currentIndex = nextIndex;
			return _historyTextures[_currentIndex]->GetId();
		}
