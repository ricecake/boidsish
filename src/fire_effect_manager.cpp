...
namespace Boidsish {
	...
	void FireEffectManager::_EnsureShaderAndBuffers() {
		...
		behavior_command_pb_ = std::make_unique<PersistentBuffer<uint32_t>>(GL_DISPATCH_INDIRECT_BUFFER, 4, 3);
	...
	void FireEffectManager::Update(
		...
		*stats_ptr = ParticleStats{};
		...
		glDispatchComputeIndirect((GLintptr)behavior_command_pb_->GetFrameOffset());
