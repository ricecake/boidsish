...
namespace Boidsish {
	...
	FireEffectManager::~FireEffectManager() {
		...
		visible_indices_pb_.reset();
		live_indices_pb_.reset();
		stats_buffer_.reset();
	...
	void FireEffectManager::_EnsureShaderAndBuffers() {
		...
		visible_indices_pb_ = std::make_unique<PersistentBuffer<uint32_t>>(GL_SHADER_STORAGE_BUFFER, kMaxParticles, 3);
		live_indices_pb_ = std::make_unique<PersistentBuffer<uint32_t>>(GL_SHADER_STORAGE_BUFFER, kMaxParticles, 3);
	...
	void FireEffectManager::Update(
		...
		// Advance and update persistent buffers
		slice_data_pb_->AdvanceFrame();
		if (!slice_points.empty()) {
			size_t copy_count = std::min(slice_points.size(), slice_data_pb_->GetElementCount());
			std::memcpy(slice_data_pb_->GetFrameDataPtr(), slice_points.data(), copy_count * sizeof(glm::vec4));
		}
		...
		if (!chunk_info.empty()) {
			size_t copy_count = std::min(chunk_info.size(), terrain_chunk_pb_->GetElementCount());
			std::memcpy(terrain_chunk_pb_->GetFrameDataPtr(), chunk_info.data(), copy_count * sizeof(glm::vec4));
		}
		...
		visible_indices_pb_->AdvanceFrame();
		live_indices_pb_->AdvanceFrame();
		...
		visible_indices_pb_->BindRange(Constants::SsboBinding::VisibleParticleIndices());
		draw_command_pb_->BindRange(Constants::SsboBinding::ParticleDrawCommand());
		live_indices_pb_->BindRange(Constants::SsboBinding::LiveParticleIndices());
