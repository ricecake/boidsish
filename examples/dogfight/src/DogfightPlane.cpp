...
namespace Boidsish {
	...
	void DogfightPlane::UpdateEntity(const EntityHandler& handler, float time, float delta_time) {
		...
		glm::vec3 torque = CalculateSteeringTorque(local_fwd, desired_dir_local, rigid_body_.GetAngularVelocity(), 50.0f, 6.0f);
	...
	void DogfightPlane::Explode(const EntityHandler& handler) {
		...
		handler.EnqueueVisualizerAction([=, &handler]() {
			handler.vis->AddFireEffect(pos, FireEffectStyle::Explosion, glm::vec3(0, 1, 0), glm::vec3(0, 0, 0), -1, 2.0f);
