...
namespace Boidsish {
	...
	void GuidedMissileLauncher::UpdateEntity(const EntityHandler& handler, float time, float delta_time) {
		...
		glm::vec3 pvel_n = (glm::length(pvel) > 0.001f) ? glm::normalize(pvel) : glm::vec3(0, 0, 1);
		glm::vec3 to_launcher_n = (glm::length(pos - ppos) > 0.001f) ? glm::normalize(pos - ppos) : glm::vec3(0, 1, 0);
