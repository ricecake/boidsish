...
public:
	...
	void UpdateEntity(const EntityHandler& /*handler*/, float /*time*/, float delta_time) override {
		...
		if (input.pitch_up) target_rot_velocity.x += kPitchSpeed;
		if (input.pitch_down) target_rot_velocity.x -= kPitchSpeed;
		if (input.yaw_left) target_rot_velocity.y += kYawSpeed;
		if (input.yaw_right) target_rot_velocity.y -= kYawSpeed;
		if (input.roll_left) target_rot_velocity.z += kRollSpeed;
		if (input.roll_right) target_rot_velocity.z -= kRollSpeed;
		...
		if (input.boost) forward_speed_ = glm::mix(forward_speed_, 120.0f, 1.0f - exp(-delta_time * 2.0f));
		else if (input.brake) forward_speed_ = glm::mix(forward_speed_, 10.0f, 1.0f - exp(-delta_time * 2.0f));
		else forward_speed_ = glm::mix(forward_speed_, 60.0f, 1.0f - exp(-delta_time * 1.0f));
