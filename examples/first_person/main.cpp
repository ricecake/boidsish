...
int main() {
	try {
		...
		viz.AddInputCallback([&](const InputState& state) {
			...
			if (state.keys[GLFW_KEY_W]) moveDir += front;
			if (state.keys[GLFW_KEY_S]) moveDir -= front;
			if (state.keys[GLFW_KEY_A]) moveDir -= right;
			if (state.keys[GLFW_KEY_D]) moveDir += right;
			...
			rig->Update(camera.pos(), camera.front(), camera.up(),
						 dt, bobAmount, bobCycle,
