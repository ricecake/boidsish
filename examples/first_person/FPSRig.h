...
public:
	FPSRig(const std::string& modelPath) {
		...
		model_->SetBaseRotation(glm::angleAxis(glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f)));
	}
	 ...
	 */
	void Update(const glm::vec3& camPos, const glm::vec3& camFront, const glm::vec3& camUp,
				float deltaTime, float bobAmount, float bobCycle, float mouseDeltaX, float mouseDeltaY) {
		...
		glm::vec3 baseOffset = right * 0.15f - actualUp * 0.12f + camFront * 0.25f;
