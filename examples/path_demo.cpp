...
int main(int argc, char** argv) {
    try {
        ...
        auto entity_handler = std::make_shared<EntityHandler>();
        for (int i = 0; i < 10; ++i) {
        ...
        vis.AddShapeHandler([entity_handler](float time) {
            return (*entity_handler)(time);
        });

        vis.AddShapeHandler([path_handler](float time) {
            return path_handler->GetShapes();
        });
