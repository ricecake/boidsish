...
void BuildGraphFromSCA(std::shared_ptr<Boidsish::Graph> graph, const std::vector<RenderNode>& scaNodes) {
	...
	// --- Pass 2: Link Edges ---
	// We iterate the SCA nodes again to reconstruct relationships
	for (const auto& node : scaNodes) {
		logger::LOG("NODE");
		// Find the ID of the current node (the parent)
		auto parentIt = posToID.find(node.position);
		if (parentIt == posToID.end())
			continue; // Should not happen

		int                      parentID = parentIt->second;
		Boidsish::Graph::Vertex& parentVertex = graph->V(parentID);

		// Link to all children
		for (const auto& childPos : node.connections) {
			auto childIt = posToID.find(childPos.position);

			if (childIt != posToID.end()) {
				int                      childID = childIt->second;
				Boidsish::Graph::Vertex& childVertex = graph->V(childID);

				// Use your Graph's Link method
				parentVertex.Link(childVertex);
			}
		}
	}

	// The graph buffers are marked 'dirty' inside AddVertex/AddEdge,
	// so the next render call will automatically rebuild the VBOs.
}
...
int main() {
	try {
		...
		// for (auto i : std::ranges::iota_view(1, 60)) {
		// 	sca.Step();
		// }
		...
		// viz.AddShapeHandler(std::ref(TreeShape));
		std::vector<std::shared_ptr<Shape>> shapes;
		shapes.push_back(graph);
		sca.Initialize();
		...
		auto topology = sca.GetTopology();

		BuildGraphFromSCA(graph, topology);
		...
		viz.AddShapeHandler([&](float time) {
			...
			// auto topo = sca.GetTopology();
			// for (auto rn : topo) {
			// 	if (rn.generation >= sca.generation) {
			// 		auto nv = graph->AddVertex(rn.position, 3, 1, 1, 1, 1);
			// 		for (auto p : rn.connections) {
			// 			auto conn = graph->V(p.id);
