...
int main() {
    ...
    high_density_props.biomes = BiomeBitset({Biome::Sand, Biome::LushGrass, Biome::DryGrass, Biome::Forest, Biome::AlpineMeadow, Biome::BrownRock, Biome::GreyRock, Biome::Snow});
    ...
    Camera camera;
    camera.x = 16; camera.y = 50; camera.z = 16;
    ...
    for (const auto& res : results) {
        if (res.instances.empty()) continue;
        found_any = true;
        ...
        for (size_t i = 0; i < std::min(res.instances.size(), size_t(2)); ++i) {
            ...
            std::cout << "  Instance " << i << ": pos(" << inst.center.x << ", " << inst.center.y << ", " << inst.center.z << ")" << std::endl;
        }
    }
