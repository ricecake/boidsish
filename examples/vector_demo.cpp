...
public:
    ...
    void UpdateEntity(float time, float delta_time) override {
        (void)time; // Mark unused parameter
        phase_ += delta_time;
...
public:
    FlockingEntity(int id, const Vector3& start_pos) : Entity(id), neighbors_() {
        SetPosition(start_pos);
        SetSize(6.0f);
        SetTrailLength(40);
    }

    void SetNeighbors(const std::vector<FlockingEntity*>& neighbors) {
...
private:
    std::vector<FlockingEntity*> neighbors_;
...
public:
    VectorDemoHandler() {
        ...
        std::vector<FlockingEntity*> flock_entities;
        for (int i = 0; i < 8; i++) {
            Vector3 start_pos(
                (rand() % 10 - 5) * 2.0f,
                (rand() % 6 - 3) * 2.0f,
            ...
            int id = AddEntity<FlockingEntity>(start_pos);
            auto* entity = dynamic_cast<FlockingEntity*>(GetEntity(id));
            if (entity) {
                flock_entities.push_back(entity);
            }
        }

        // Set up neighbor relationships for flocking
        for (auto* entity : flock_entities) {
            entity->SetNeighbors(flock_entities);
        }

        std::cout << "Created " << flock_entities.size() << " flocking entities and 5 target-seeking entities" << std::endl;
