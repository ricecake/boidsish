		...
		if (ImGui::Begin("Atmosphere Designer")) {
			...
			if (ImGui::CollapsingHeader("Lighting & Sun", ImGuiTreeNodeFlags_DefaultOpen)) {
				...
				if (!lights.empty()) {
					...
					bool changed = false;
					if (ImGui::SliderFloat("Azimuth", &sun.azimuth, 0.0f, 360.0f)) changed = true;
					if (ImGui::SliderFloat("Elevation", &sun.elevation, -90.0f, 90.0f)) changed = true;
...
int main(int argc, char** argv) {
	...
	cam.x = 0; cam.y = 2; cam.z = 5;
	cam.pitch = 15; cam.yaw = 0;
	vis.SetCamera(cam);
