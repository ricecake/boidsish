...
int main() {
	try {
		...
		auto pick_biome_target = [&](ITerrainGenerator& gen, const glm::vec3& current_pos) -> glm::vec3 {
			...
			if (!terrain_gen)
				return current_pos + glm::vec3(100, 0, 100);
		...
		auto generate_path = [&](ITerrainGenerator& gen, const glm::vec3& start, const glm::vec3& target) {
			...
			for (int i = 1; i <= segments; ++i) {
				...
				// LOS check (simple)
				glm::vec3 prev_p(
					path_state.waypoints.back().x,
					path_state.waypoints.back().y,
					path_state.waypoints.back().z
				);
		...
		visualizer.AddShapeHandler([&](float time) {
			...
			if (path_state.active && path_state.waypoints.size() >= 4) {
				...
				Vector3 p = Spline::CatmullRom(
					t,
					path_state.waypoints[i0],
					path_state.waypoints[i1],
					path_state.waypoints[i2],
					path_state.waypoints[i3]
				);
