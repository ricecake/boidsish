...
namespace Boidsish {
    ...
    void SdfBoid::UpdateEntity(const EntityHandler& handler, float time, float delta_time) {
        ...
        float max_vel = is_predator_ ? 40.0f : 30.0f;
        if (glm::length(vel) > max_vel) vel = glm::normalize(vel) * max_vel;
    ...
    SdfBoidHandler::SdfBoidHandler(task_thread_pool::task_thread_pool& thread_pool, std::shared_ptr<Visualizer>& visualizer)
        ...
        // Add boids
        for (int i = 0; i < 40; ++i) {
            bool is_predator = (i == 0); // One predator
            ...
            source.radius = is_predator ? 15.0f : 10.0f;
            source.color = is_predator ? glm::vec3(1.0f, 0.2f, 0.2f) : glm::vec3(0.2f, 0.6f, 1.0f);
            source.charge = is_predator ? -1.0f : 1.0f; // Predator cancels, others merge
