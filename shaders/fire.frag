...
void main() {
	...
	if (v_style == STYLE_ROCKET_TRAIL || v_style == STYLE_SPARKS || v_style == STYLE_GLITTER || v_style == STYLE_BUBBLES || v_style == STYLE_DEBUG ||
		...
		if (v_style == STYLE_BUBBLES) {
			...
			float ringWidth = 0.02 + twinkle * 0.08;
			...
			vec3 diffraction;
			diffraction.r = sin(angle * 3.0 + u_time * 5.0 + dist * 10.0) * 0.5 + 0.5;
			diffraction.g = sin(angle * 3.0 + u_time * 5.0 + dist * 10.0 + 2.0) * 0.5 + 0.5;
			diffraction.b = sin(angle * 3.0 + u_time * 5.0 + dist * 10.0 + 4.0) * 0.5 + 0.5;

			color = mix(color, color * diffraction * 2.0, twinkle);
