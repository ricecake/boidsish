...
// Robust polynomial fit for HDR-friendly fire with physically grounded units
vec3 blackbody_hdr(float t) {
	...
	// Physically grounded flame luminance (25.0, 10.0, 5.0 HDR intensity scale)
	return col * vec3(25.0, 10.0, 5.0);
}
...
void main() {
	...
	// View matrix inverse to transform view-space billboard normal to world space
	mat3 invView = transpose(mat3(u_view));
	...
	if (v_style == STYLE_ROCKET_TRAIL || v_style == STYLE_SPARKS || v_style == STYLE_GLITTER || v_style == STYLE_BUBBLES || v_style == STYLE_DEBUG ||
		...
		if (v_style == STYLE_ROCKET_TRAIL) {
			isEmissive = true;
			color *= 35.0; // Rebalanced physical HDR exhaust emission
		} else if (v_style == STYLE_SPARKS) {
			isEmissive = true;
			color *= 25.0; // Rebalanced physical HDR spark emission
		} else if (v_style == STYLE_GLITTER) {
			isEmissive = true;
			color *= 20.0; // Rebalanced physical HDR glitter emission
		} else if (v_style == STYLE_FIREFLIES) {
			isEmissive = true;
			color *= 12.0; // Rebalanced physical HDR bioluminescent firefly glow
			...
			color *= 15.0; // Rebalanced physical HDR fairy glow intensity
			alpha = v_p.color.a * shapeMask;
			isEmissive = true;
		} else if (v_style == STYLE_BUBBLES) {
			n_view.xy = circ * 2.0;
			float magSq = dot(n_view.xy, n_view.xy);
			n_view.z = sqrt(max(0.0, 1.0 - magSq));
		...
		color = blackbody_hdr(heat) * alpha * (1.0 + normalizedLife);
	}

	// Calculate PBR lighting for non-emissive particles using consistent system
	if (!isEmissive && alpha > 0.001) {
		vec3 N = normalize(invView * n_view);
		float dummyShadow = 1.0;
		vec3 litColor = color;

		if (v_style == STYLE_LEAF) {
			litColor = apply_lighting_foliage(v_pos.xyz, N, color, 0.5, 0.0, 1.0, dummyShadow).rgb;
		} else if (v_style == STYLE_PETAL) {
			litColor = apply_lighting_foliage(v_pos.xyz, N, color, 0.4, 0.0, 1.0, dummyShadow).rgb;
		} else if (v_style == STYLE_BIRDS) {
			litColor = apply_lighting_pbr_no_shadows(v_pos.xyz, N, color, 0.7, 0.0, 1.0, dummyShadow).rgb;
		} else if (v_style == STYLE_BUBBLES) {
			litColor = apply_lighting_pbr_iridescent_no_shadows(v_pos.xyz, N, color, 0.05, 0.8, dummyShadow).rgb;
		} else if (v_style == STYLE_IRIDESCENT) {
			litColor = apply_lighting_pbr_iridescent_no_shadows(v_pos.xyz, N, color, 0.05, 1.0, dummyShadow).rgb;
		} else if (v_style == STYLE_SNOW) {
			litColor = apply_lighting_pbr_no_shadows(v_pos.xyz, N, vec3(0.95), 0.3, 0.0, 1.0, dummyShadow, 0.5).rgb;
		} else if (v_style == STYLE_RAIN) {
			litColor = apply_lighting_pbr_no_shadows(v_pos.xyz, N, color, 0.05, 0.1, 1.0, dummyShadow).rgb;
		} else if (v_style == STYLE_DUST) {
			litColor = apply_lighting_pbr_no_shadows(v_pos.xyz, N, color, 0.8, 0.0, 1.0, dummyShadow).rgb;
		} else if (v_style == STYLE_CINDER) {
			litColor = apply_lighting_pbr_no_shadows(v_pos.xyz, N, color, 0.8, 0.0, 1.0, dummyShadow).rgb;
		} else if (v_style == STYLE_POOF) {
			litColor = apply_lighting_pbr_no_shadows(v_pos.xyz, N, color, 0.95, 0.0, 1.0, dummyShadow).rgb;
		} else {
			litColor = apply_lighting_pbr_no_shadows(v_pos.xyz, N, color, 0.5, 0.0, 1.0, dummyShadow).rgb;
		}
	...
	float transmittance = exp(-length(s.extinction) * (depth / 1000.0));
