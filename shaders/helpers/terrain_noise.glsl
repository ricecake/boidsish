...
// curlVec: The sampled vector from your baked curl texture at 'pos'
...
float tangentGabor(vec3 worldPos, vec3 worldNormal, vec3 curlVec3D, float time, float freq, float bandwidth, float sparsity) {
	// Inputs: worldPos (vec3), worldNormal (vec3), curlVec3D (vec3)
