...
vec3 calculateMetaBiomeWeights(vec2 worldXZ, float controlNoiseScale) {
	vec2 pos = worldXZ * controlNoiseScale * 0.1;
	...
	for (int j = -1; j <= 1; j++) {
		for (int i = -1; i <= 1; i++) {
			...
			vec2  r_pos = p + b;
			float h = snoise(r_pos);
			vec2  r = b - f + (h * 0.5 + 0.5);
			float d = dot(r, r);

			int biome_id = int(abs(r_pos.x * 7.0 + r_pos.y * 13.0)) % 3;
	...
	float t = smoothstep(0.0, edge_width, f2 - f1);
