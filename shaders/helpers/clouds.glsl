...
CloudSpotDetails calculateCloudDensityExpV9(
	...
	float penumbra = 500.0 * props.worldScale * max(0.1, cloudShadowStepMultiplier);
	float baseNoise = clamp(-baseSdf / penumbra, 0.0, 1.0);
...
CloudDensityResult calculateCloudDensityMinimal(
	...
	return CloudDensityResult(vec3(volDensity), advectSpeed, 1.0, vec3(1.0), vec3(0.0));
}
...
CloudDensityResult calculateCloudDensity(
	...
	vec3 mixedDensity = vec3(volDensity);
