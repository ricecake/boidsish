...
float calculateCloudShadowFactor(vec3 frag_pos, vec3 L, float intensity) {
	...
	float accumulatedDensity = sampleDeepOpacityMap(shadowUV, h, 1.5) * 0.001 * cloudShadowOpticalDepthMultiplier / max(0.001, worldScale);
...
float evaluateCloudShadowDensityAtWorldPos(vec2 worldXZ, float time) {
	...
	float totalDensity = textureLod(u_cloudShadowTexture, vec3(shadowUV, 7.0), 1.5).r * 0.001 * cloudShadowOpticalDepthMultiplier / max(0.001, worldScale);
	return totalDensity;
}
...
float calculateCloudAmbientOcclusion(vec3 frag_pos) {
	...
	float cloudAO = exp(-accumulatedDensity * 0.5); // scaled down for softer ambient occlusion
