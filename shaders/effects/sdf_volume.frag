...
vec3 getNormal(vec3 p) {
	...
	float len = length(n);
	return len > 1e-6 ? n / len : vec3(0, 1, 0);
}
...
void main() {
	...
	float distToScene = length(worldPos.xyz - cameraPos);
	if (depth >= 0.999999)
		distToScene = 1500.0;

	// Robust ray direction reconstruction
	vec4 target = invProjection * vec4(TexCoords * 2.0 - 1.0, 1.0, 1.0);
	vec3 rayDir = normalize((invView * vec4(normalize(target.xyz), 0.0)).xyz);

	float t = 0.0;

	// Global AABB check to skip empty space
	vec3 boxCenter = (sdfMin + sdfMax) * 0.5;
	vec3 boxHalfExtents = (sdfMax - sdfMin) * 0.5 + vec3(15.0);
	...
	// Raymarching loop with safety break and positive progress
	for (int i = 0; i < 96; ++i) {
		if (t > distToScene || t > 1500.0)
			break;
	...
	// Double check we didn't overshoot the scene
	if (hit && t > distToScene) {
		hit = false;
	}
