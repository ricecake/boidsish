...
struct SdfSource {
    ...
    vec4 prim_params;          // xyz: size, w: height
};
...
vec4 opUnionColored(vec4 d1, vec4 d2, float k) {
	float h = clamp(0.5 + 0.5 * (d2.a - d1.a) / (k + 1e-6), 0.0, 1.0);
...
float getSdf(vec3 p, int i) {
    int type = int(sources[i].charge_type_vol_unused.y);
    ...
    if (type == 0) { // Sphere
        ...
        return boxSDF(p - center, sources[i].prim_params.xyz);
    } else if (type == 2) { // Capsule
        // We use radius for the radius, and prim_params.w for the height
        float h = sources[i].prim_params.w;
        vec3 a = center - vec3(0.0, h * 0.5, 0.0);
        vec3 b = center + vec3(0.0, h * 0.5, 0.0);
        return capsuleSDF(p, a, b, radius);
    }
    return 1e10;
}
...
float mapDistance(vec3 p) {
	...
	for (int i = 0; i < numSources; ++i) {
		if (sources[i].charge_type_vol_unused.x > 0.0 && sources[i].charge_type_vol_unused.z < 0.5) {
			float d_src = getSdf(p, i);
            if (d > 1e9) d = d_src;
			else {
                float k = sources[i].color_smoothness.a;
                float h = clamp(0.5 + 0.5 * (d - d_src) / (k + 1e-6), 0.0, 1.0);
...
void main() {
	...
	vec3 rayDir = normalize((invView * vec4(normalize(target.xyz + 1e-6), 0.0)).xyz);
    ...
    for (int i = 0; i < numSources; ++i) {
        if (sources[i].charge_type_vol_unused.z > 0.5) { // Volumetric
            vec3 center = sources[i].position_radius.xyz;
            float radius = sources[i].position_radius.w * 2.0; // Bounding sphere slightly larger
                ...
                if (t_start < t_end) {
                    ...
                    for (int j = 0; j < steps; ++j) {
                        ...
                        if (d < 0.0) {
                            ...
                            float normalized_d = clamp(-d / (sources[i].position_radius.w + 1e-6), 0.0, 1.0);
    ...
    vec2 prevTexCoords = (reprojectedPos.xy / (reprojectedPos.w + 1e-6)) * 0.5 + 0.5;
    ...
    // Only use history if it's not the first frame and we are on screen
    float blendFactor = (onScreen && frameIndex > 0) ? 0.9 : 0.0;

    // Safety check for historyColor validity
    if (any(isnan(historyColor)) || any(isinf(historyColor))) blendFactor = 0.0;

    FragColor = mix(vec4(currentFrameColor, 1.0), historyColor, blendFactor);

    // Ensure final color is not NaN/Inf
    if (any(isnan(FragColor)) || any(isinf(FragColor))) FragColor = vec4(currentFrameColor, 1.0);
}
