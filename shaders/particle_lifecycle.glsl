...
bool updateLifetime(inout Particle p, uint gid, int emitter_index, int num_emitters, float dt, vec3 viewPos, vec3 viewDir) {
	// Hijack/Clear Logic
...
void respawnParticle(inout Particle p, uint gid, int emitter_index, int num_emitters, float time, float dt, float ambient_density, int num_chunks, vec3 viewPos, vec3 viewDir, float nightFactor, sampler3D curlTexture, sampler2DArray heightmapArray, sampler2DArray biomeMap, float cellSize, uint gridSize) {
	if (p.pos.w <= 0.0) {
		...
		if (emitter_index != -1 && num_emitters > 0 && emitter_index < num_emitters) {
			...
			if (emitter.is_active != 0) {
				...
				if (emitter.use_slice_data != 0) {
					...
					if (!skip_spawn) spawn_pos += rel_p;
				} else if (emitter.type == 2) { // Sphere
					vec3 r = normalize(random_vec) * rand(seed.yx);
					if (r.y > (emitter.sweep * 2.0 - 1.0)) {
						skip_spawn = true;
					}
					if (!skip_spawn) spawn_pos += r * emitter.dimensions.x;
				} else if (emitter.type == 3) { // Beam
					float r = rand(seed);
					if (r > emitter.sweep) {
						skip_spawn = true;
					}
					if (!skip_spawn) spawn_pos += normalize(emitter.direction) * r * emitter.dimensions.x;
				}
				...
				if (!skip_spawn) {
					if (p.style == 0) { // MissileExhaust
						...
						p.vel = vec4(normalize(random_vec) * kExplosionSpeed * 0.5 + abs(fbmCurlMagnitude(p.pos.xyz, dt, curlTexture)), 0.0);
