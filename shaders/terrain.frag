...
void main() {
	...
	vec2 warp_in = FragPos.xz + time * 0.1;
	float warp_noise = fbm(warp_in);
	vec2 warp = vec2(warp_noise, warp_noise);
	float nebula_noise = fbm(FragPos.xz + warp * 0.5);
	// objectColor = nebula_noise * (warp + objectColor);
	vec2 warpNoise = nebula_noise * warp;
	objectColor.xy += warpNoise;
	...
	vec2  warp2_in = fwidth(FragPos.xz) + time * 0.1;
	float warp2_noise = fbm(warp2_in);
	vec2 warp2 = vec2(warp2_noise, warp2_noise);
	float nebula_noise2 = fbm(fwidth(FragPos.xz) + warp * 0.5);
	...
	vec3  grid_color = normalize(abs(fwidth(FragPos.zxy))) * intensity;
    grid_color.xy += nebula_noise2*warp2;
