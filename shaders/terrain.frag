...
void main() {
	...
	vec3 advectDir = realDist > 0.001 ? (FragPos - viewPos) / realDist : vec3(0.0, 0.0, 1.0);

	// Proper 3D advected and warped lookups
	vec3 warpCoord1 = FragPos * (0.005 / worldScale) + vec3(0.0, time * 0.05, 0.0);
	vec3 warp1 = fastCurl3d(warpCoord1) * 80.0 * worldScale;
	float speed1 = 25.0 * worldScale;
	vec3 p1 = FragPos + (advectDir * time * speed1) + warp1;
	...
	vec3 warp2 = fastCurl3d(warpCoord2) * 40.0 * worldScale;
	...
	float nightNoise = fastWorley3d(p2 / (50.0 * worldScale));
	...
	float temperatureFactor = smoothstep(0.0, 0.8, nightFade);
	...
	vec3 printFrontColor = mix(cyan, magenta, 0.5) * 3.0 * printFront;
