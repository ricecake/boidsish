...
void updateAmbientParticle(inout Particle p, float dt, float time, vec3 viewPos, vec3 viewDir, float cellSize, uint gridSize, sampler3D curlTexture, int num_chunks, sampler2DArray heightmapArray) {
	float maxSpeed = 2.0;
...
void updateFireBehavior(inout Particle p, float dt, float time, sampler3D curlTexture, int num_chunks, sampler2DArray heightmapArray) {
	float maxSpeed = 10.0;
	if (p.style == 0) { // MissileExhaust
		...
		float curlInfluence = smoothstep(5.0, 30.0, dist) +
			smoothstep(5, 1, length(p.vel.xyz) * kExplosionDrag * dt);
		...
		float dist = distance(p.pos.xyz, p.epicenter);
		float curlInfluence = smoothstep(5.0, 30.0, dist) +
			smoothstep(5, 1, length(p.vel.xyz) * kGlitterDrag * dt);
...
void updateBehavior(inout Particle p, float dt, float time, vec3 viewPos, vec3 viewDir, float cellSize, uint gridSize, sampler3D curlTexture, int num_chunks, sampler2DArray heightmapArray) {
	if (p.style == 5) {
		updateAmbientParticle(p, dt, time, viewPos, viewDir, cellSize, gridSize, curlTexture, num_chunks, heightmapArray);
	} else {
