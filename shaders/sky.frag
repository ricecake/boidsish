...
void main() {
	...
	float aureoleNoise = pow((fbm_sky(vec3(rayLocalX*3, rayLocalY*3, time*0.01)) * 0.5) + 1.5, 3)/(1+distSq);
	...
	if (starVisibility > 0.0) {
		...
		// vec3  nebula = mix(vec3(0.0, 0.0, 0.0), vec3(0.8, 0.2, 0.7), nebula_noise) * 0.24;
		nebula = mix(vec3(0.0, 0.0, 0.0), vec3(0.8, 0.2, 0.7)*curl(warp_offset), nebula_noise) * 0.1 * snoise(warp_offset);
		stars = starLayer(world_ray) * vec3(1.0, 0.9, 0.8);
	}
