...
vec3 sampleSkyView(vec3 rd) {
    ...
    float v = (elevation < 0.0) ? (0.5 - 0.5 * sqrt(-elevation / (PI * 0.5))) : (0.5 + 0.5 * sqrt(elevation / (PI * 0.5)));
...
void main() {
    ...
    float sunMask = smoothstep(sunAngularRadius * sunAngularRadius, (sunAngularRadius - 0.001) * (sunAngularRadius - 0.001), distSq);
