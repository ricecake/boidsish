...
vec2 calc_optical_flow(vec2 uv) {
    ...
    // Gradient in X
    float gradX = luma(texture(u_current_texture, uv + offset_vec.xy).rgb) - luma(texture(u_current_texture, uv - offset_vec.xy).rgb);
    gradX += luma(texture(u_previous_texture, uv + offset_vec.xy).rgb) - luma(texture(u_previous_texture, uv - offset_vec.xy).rgb);

    // Gradient in Y
    float gradY = luma(texture(u_current_texture, uv + offset_vec.yx).rgb) - luma(texture(u_current_texture, uv - offset_vec.yx).rgb);
    gradY += luma(texture(u_previous_texture, uv + offset_vec.yx).rgb) - luma(texture(u_previous_texture, uv - offset_vec.yx).rgb);
...
void main() {
    ...
    // out_acceleration = vec4(fwidth(current_flow.x), fwidth(current_flow.y), fwidth(previous_flow.x), fwidth(previous_flow.y));
    ...
    // out_acceleration = vec4(texture(u_current_texture, v_tex_coords).rgb, 1.0) + vec4(vec3(acceleration_magnitude), 1.0);
    // out_acceleration = vec4(vec3(acceleration_magnitude), 1.0);
