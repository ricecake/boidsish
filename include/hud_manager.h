...
namespace Boidsish {
	...
	public:
		...
		// Modern API
		std::shared_ptr<HudIcon>   AddIcon(const std::string& path, HudAlignment alignment = HudAlignment::TOP_LEFT, glm::vec2 position = {0, 0}, glm::vec2 size = {64, 64});
		std::shared_ptr<HudNumber> AddNumber(float value = 0.0f, const std::string& label = "", HudAlignment alignment = HudAlignment::TOP_RIGHT, glm::vec2 position = {-10, 10}, int precision = 2);
		std::shared_ptr<HudGauge>  AddGauge(float value = 0.0f, const std::string& label = "", HudAlignment alignment = HudAlignment::BOTTOM_CENTER, glm::vec2 position = {0, -50}, glm::vec2 size = {200, 20});
