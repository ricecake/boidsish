...
namespace Boidsish {
	...
	private:
		...
		// Triple-buffered persistent buffers for command and transient data
		...
		std::unique_ptr<PersistentBuffer<uint32_t>>  visible_indices_pb_;
		std::unique_ptr<PersistentBuffer<uint32_t>>  live_indices_pb_;
