...
public:
	...
	static void
	compute_heatmap(const std::vector<uint32_t>& targets, float* heatmap, size_t heatmap_size, int max_bfs_depth = 32) {
		if (!heatmap)
			return;
		...
		while (!q.empty()) {
			...
			for (uint32_t i = 0; i < node.edge_count; ++i) {
				...
				if (edge.target_id < heatmap_size) {
					if (heatmap[edge.target_id] == 0.0f) {
						q.push({edge.target_id, current.depth + 1});
					}
					// Decay based on depth
					// heatmap[edge.target_id] += 1.0f / (current.depth * 0.5f);
					float target_depth = static_cast<float>(current.depth + 1);
					// heatmap[edge.target_id] += 1.0f / (target_depth * 0.5f);
					heatmap[edge.target_id] += sqrt(heatmap[current.id]);
				}
			}
		}
	}
...
private:
	struct RuleSet {
		...
		bool                               is_lexicon = false; // Distinguishes terminal-producing rules
	};
			...
			for (size_t i = 0; i < ruleset.productions.size(); ++i) {
				...
				if (prod.size() == 1) {
					...
					for (size_t j = 0; j < nts.size() - 2; ++j) {
						uint32_t synth_id = interner_.get_or_intern(
							"__synth_" + std::to_string(lhs) + "_" + std::to_string(i) + "_" + std::to_string(j)
						);
						// Only the first binarized segment carries the original rule link to avoid exponential
						// weighting
						binarized_grammar_.push_back(
							{current_lhs,
						     nts[j],
						     synth_id,
						     (j == 0 ? log_prob : 0.0),
					...
					binarized_grammar_.push_back(
						{current_lhs,
					     nts[nts.size() - 2],
					     nts[nts.size() - 1],
					     (nts.size() == 2 ? log_prob : 0.0),
					     false,
					     (nts.size() == 2 ? lhs : 0),
					     (nts.size() == 2 ? static_cast<int>(i) : -1)}
					);
				}
			}
		}
	...
	void generate_recursive(uint32_t symbol_id, GenerationContext& ctx, std::string& output) {
		...
		for (size_t i = 0; i < it->second.base_weights.size(); ++i) {
			...
			// Only apply semantic heatmap boost to terminal productions (lexicon)
			if (it->second.is_lexicon && ctx.semantic_heatmap) {
	...
	void load_rules(const std::string& filename, bool is_lexicon) {
		std::ifstream file(filename);
		if (!file.is_open())
		...
		while (std::getline(file, line)) {
			...
			grammar_[lhs_id].is_lexicon = is_lexicon;
		}
	}

	void dump_rules_by_type(std::ostream& out, bool dump_lexicon) const {
		for (const auto& [lhs_id, ruleset] : grammar_) {
			if (ruleset.is_lexicon != dump_lexicon)
				continue;

			for (size_t i = 0; i < ruleset.productions.size(); ++i) {
				out << ruleset.base_weights[i] << " " << interner_.get_string(lhs_id);
				for (uint32_t rhs_id : ruleset.productions[i]) {
					out << " " << interner_.get_string(rhs_id);
				}
				out << "\n";
			}
		}
...
public:
	explicit Grammar(const std::string& structural_file, const std::string& terminal_file) {
		std::random_device rd;
		rng_ = std::mt19937(rd());

		load_rules(structural_file, false);
		load_rules(terminal_file, true);
	}
	...
	std::vector<uint32_t> parse_and_modify(const std::string& sentence, const std::string& start = "ROOT") {
		...
		std::vector<std::vector<std::unordered_map<uint32_t, Entry>>> table(
			n,
			std::vector<std::unordered_map<uint32_t, Entry>>(n)
		);
	...
	void dump_rules(const std::string& filename) const {
		std::ofstream out(filename);
		if (!out.is_open())
			throw std::runtime_error("Could not open file for dumping: " + filename);

		out << "# --- STRUCTURAL RULES ---\n";
		dump_rules_by_type(out, false);

		out << "\n# --- TERMINAL RULES (LEXICON) ---\n";
		dump_rules_by_type(out, true);
	}
};
