...
public:
    ...
    explicit RenderShader(std::shared_ptr<ShaderBase> backingShader)
        : m_backingShader(std::move(backingShader)) {}
     ...
     */
    void Flush() {
        if (!m_backingShader || m_pendingUniforms.empty()) return;
...
protected:
    ...
    void ApplyUniform(const std::string& name, const ShaderBase::UniformValue& value) {
        std::visit([&](auto&& arg) {
            ...
            else if constexpr (std::is_same_v<T, bool>) m_backingShader->setBool(name, arg);
            else if constexpr (std::is_same_v<T, int>) m_backingShader->setInt(name, arg);
            else if constexpr (std::is_same_v<T, float>) m_backingShader->setFloat(name, arg);
            else if constexpr (std::is_same_v<T, glm::vec2>) m_backingShader->setVec2(name, arg);
            else if constexpr (std::is_same_v<T, glm::vec3>) m_backingShader->setVec3(name, arg);
            else if constexpr (std::is_same_v<T, glm::vec4>) m_backingShader->setVec4(name, arg);
            else if constexpr (std::is_same_v<T, glm::mat2>) m_backingShader->setMat2(name, arg);
            else if constexpr (std::is_same_v<T, glm::mat3>) m_backingShader->setMat3(name, arg);
            else if constexpr (std::is_same_v<T, glm::mat4>) m_backingShader->setMat4(name, arg);
            else if constexpr (std::is_same_v<T, std::vector<int>>) m_backingShader->setIntArray(name, arg.data(), static_cast<int>(arg.size()));
        }, value.value);
    }
